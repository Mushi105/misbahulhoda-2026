  ...
  <div class="min-h-screen bg-dark-950 p-4 md:p-6 space-y-6">
    ...
    <template v-else-if="profile && !showProfileForm">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card space-y-3">
          <h2 class="text-base font-semibold text-white border-b border-dark-600 pb-2">Travel & Allocation</h2>
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div><p class="text-slate-500 text-xs">Arrival</p><p class="text-white">{{ new Date(profile.arrivalDate).toLocaleDateString('en-GB') }}</p></div>
            <div><p class="text-slate-500 text-xs">Departure</p><p class="text-white">{{ new Date(profile.departureDate).toLocaleDateString('en-GB') }}</p></div>
            <div><p class="text-slate-500 text-xs">Arrival Flight</p><p class="text-white">{{ profile.arrivalFlight || '—' }}</p></div>
            <div><p class="text-slate-500 text-xs">Departure Flight</p><p class="text-white">{{ profile.departureFlight || '—' }}</p></div>
            <div>
              <p class="text-slate-500 text-xs">Bus</p>
              <p v-if="profile.bus" class="text-primary-400 font-semibold">Bus {{ profile.bus?.busNumber }}</p>
              <p v-else class="text-slate-500 text-xs italic">Not allocated yet</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Hotel & Room Card -->
      <div v-if="profile.room" class="rounded-xl border border-primary-800 bg-gradient-to-br from-primary-950 via-dark-900 to-dark-950 overflow-hidden">
        <div class="px-5 py-3 bg-primary-900/40 border-b border-primary-800 flex items-center gap-2">
          <span class="text-xl">🏨</span>
          <h2 class="text-base font-semibold text-white">Your Hotel & Room</h2>
        </div>
        <div class="p-5 space-y-4">
          <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p class="text-white text-lg font-bold leading-tight">{{ profile.room.hotelName || 'Hotel' }}</p>
              <p class="text-slate-400 text-sm mt-0.5">{{ profile.room.hotelCity }}</p>
              <p class="text-slate-500 text-xs mt-1">{{ profile.room.hotelAddress }}</p>
            </div>
            <div class="text-right shrink-0">
              <p class="text-slate-500 text-xs">Room Number</p>
              <p class="text-primary-400 font-bold text-xl">{{ profile.room.roomNumber }}</p>
              <p class="text-slate-500 text-xs">{{ profile.room.floorLabel || `Floor ${profile.room.floorNumber}` }}</p>
            </div>
          </div>
          <div class="flex flex-wrap gap-3 pt-1">
            <a
              v-if="profile.room.hotelPhone"
              :href="`tel:${profile.room.hotelPhone}`"
              class="flex items-center gap-2 bg-dark-800 hover:bg-dark-700 border border-dark-600 text-white text-sm px-4 py-2 rounded-lg transition-colors">
              📞 {{ profile.room.hotelPhone }}
            </a>
            <a
              v-if="profile.room.latitude && profile.room.longitude"
              :href="`https://www.google.com/maps?q=${profile.room.latitude},${profile.room.longitude}`"
              target="_blank"
              rel="noopener noreferrer"
              class="flex items-center gap-2 bg-green-900 hover:bg-green-800 border border-green-700 text-green-300 text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
              📍 Open in Google Maps
            </a>
            <button
              @click="showCheckoutConfirm = true"
              class="flex items-center gap-2 bg-red-950 hover:bg-red-900 border border-red-800 text-red-300 text-sm px-4 py-2 rounded-lg transition-colors ml-auto">
              🚪 Check Out
            </button>
          </div>
        </div>

...
async function load() {
  ...
  selected.value = new Set()
  try {
    const res = await adminApi.getPilgrims({ search: search.value || undefined, status: statusFilter.value || undefined, page: page.value, pageSize: 20 })
    ...
    <div v-else class="space-y-3">
      ...
      <div class="flex items-center gap-3 px-1">
        ...
        <span class="text-slate-400 text-sm">
          ...
          <span class="text-slate-500">({{ pilgrims.length }})</span>
        </span>
      </div>

      <div v-for="p in pilgrims" :key="p.id"
           :class="['card transition-all', selected.has(p.id) ? 'border-gold-700 bg-gold-950/20' : '']">
        <div class="flex flex-col sm:flex-row sm:items-start gap-4">
          <!-- Checkbox + Avatar + Info -->
          <div class="flex gap-3 flex-1">
            <input type="checkbox" :checked="selected.has(p.id)" @change="toggleSelect(p.id)"
              class="w-4 h-4 mt-1 accent-gold-500 cursor-pointer shrink-0" />
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2 mb-1">
                <p class="text-white font-semibold">{{ p.fullName }}</p>
                <span :class="statusStyle[p.status] || 'bg-slate-800 text-slate-400'" class="px-2 py-0.5 rounded-full text-xs font-medium">
                  {{ p.status }}
                </span>
              </div>
              ...
              <!-- Details grid -->
              <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-1.5 text-xs">
                <span class="text-slate-500">🛂 Passport: <span class="text-slate-300">{{ p.passportNumber || '—' }}</span></span>
                <span class="text-slate-500">🌍 Country: <span class="text-slate-300">{{ p.country || '—' }}</span></span>
                <span class="text-slate-500">✈ Arrival: <span class="text-slate-300">{{ p.arrivalDate ? new Date(p.arrivalDate).toLocaleDateString('en-GB') : '—' }}</span></span>
                <span class="text-slate-500">👨‍👩‍👧 Family: <span class="text-slate-300">{{ p.familyMemberCount || 0 }} members</span></span>

                <!-- Room details -->
                <span v-if="p.roomId && p.roomNumber" class="col-span-2 sm:col-span-2">
                  <span class="text-slate-500">🛏 Room: </span>
                  <span class="text-emerald-400 font-semibold">{{ p.roomNumber }}</span>
                  <span v-if="p.hotelName" class="text-slate-400"> · {{ p.hotelName }}</span>
                  <span v-if="p.floorLabel" class="text-slate-500"> · {{ p.floorLabel }}</span>
                </span>
                <span v-else-if="p.roomId" class="text-slate-500">🛏 Room: <span class="text-emerald-400">Allocated</span></span>
                <span v-else class="text-slate-500">🛏 Room: <span class="text-yellow-400">Not Assigned</span></span>

                <!-- Bus details -->
                <span v-if="p.busId && p.busNumber" class="text-slate-500">
                  🚌 Bus: <span class="text-emerald-400 font-semibold">{{ p.busNumber }}</span>
                </span>
                <span v-else-if="p.busId" class="text-slate-500">🚌 Bus: <span class="text-emerald-400">Allocated</span></span>
                <span v-else class="text-slate-500">🚌 Bus: <span class="text-yellow-400">Not Assigned</span></span>

                <span v-if="p.rejectionReason" class="col-span-2 text-red-400">❌ Reason: {{ p.rejectionReason }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-wrap gap-2 shrink-0">
            <button @click="openDetail(p)"
              class="bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs px-3 py-1.5 rounded-lg transition-colors font-medium border border-slate-700">
              📋 Details
            </button>
            <button v-if="p.status === 'Pending'"
              @click="markUnderReview(p)"
              :disabled="saving === p.id + '-review'"
              class="bg-blue-900/60 hover:bg-blue-800 text-blue-300 text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 font-medium">
              {{ saving === p.id + '-review' ? '...' : '🔍 Review' }}
            </button>
            <button v-if="p.status === 'Pending' || p.status === 'UnderReview'"
              @click="approve(p)"
              :disabled="saving === p.id + '-approve'"
              class="bg-emerald-800 hover:bg-emerald-700 text-emerald-200 text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 font-medium">
              {{ saving === p.id + '-approve' ? '...' : '✅ Approve' }}
            </button>
            <button v-if="p.status !== 'Rejected'"
              @click="openReject(p)"
              class="bg-red-900/60 hover:bg-red-800 text-red-300 text-xs px-3 py-1.5 rounded-lg transition-colors font-medium">
              ❌ Reject
            </button>
            <button v-if="p.status === 'Rejected'"
              @click="resetToPending(p)"
              :disabled="saving === p.id + '-reset'"
              class="bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 font-medium">
              {{ saving === p.id + '-reset' ? '...' : '↩ Reset' }}
            </button>
            <button v-if="p.status === 'Approved'"
              @click="openRoomModal(p)"
              :class="p.roomId ? 'bg-primary-900/60 text-primary-300' : 'bg-yellow-900/60 text-yellow-300'"
              class="hover:opacity-80 text-xs px-3 py-1.5 rounded-lg transition-colors font-medium">
              🛏 {{ p.roomId ? 'Change Room' : 'Assign Room' }}
            </button>
            <button v-if="p.status === 'Approved'"
              @click="openBusModal(p)"
              :class="p.busId ? 'bg-primary-900/60 text-primary-300' : 'bg-yellow-900/60 text-yellow-300'"
              class="hover:opacity-80 text-xs px-3 py-1.5 rounded-lg transition-colors font-medium">
              🚌 {{ p.busId ? 'Change Bus' : 'Assign Bus' }}
            </button>
          </div>
        </div>
      </div>

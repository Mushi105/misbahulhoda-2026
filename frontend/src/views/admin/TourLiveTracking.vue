<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">🛰️ Tour Live Monitor</h1>
        <p class="text-gray-700 text-sm mt-1">Track pilgrims through the Arbaeen 2026 journey day-by-day</p>
      </div>
      <div class="flex gap-2 items-center flex-wrap">
        ...
        <button @click="openScheduleSetup" v-if="selectedTourId"
          class="px-3 py-2 rounded-lg border border-amber-300 text-amber-700 bg-amber-50 text-sm hover:bg-amber-100 transition">
          📅 Setup Schedule
        </button>
        <button @click="sendReminder" :disabled="reminderSending" v-if="selectedTourId"
          class="px-3 py-2 rounded-lg border border-blue-300 text-blue-600 bg-blue-50 text-sm hover:bg-blue-100 transition disabled:opacity-50">
          {{ reminderSending ? '...' : '📢 Send Reminder' }}
        </button>
        <button @click="load" :disabled="loading"
          class="px-3 py-2 rounded-lg border border-gray-300 text-gray-600 bg-gray-50 text-sm hover:bg-gray-100 transition disabled:opacity-50">
          {{ loading ? '⏳' : '🔄' }}
        </button>
      </div>
    </div>
    ...
    <!-- Alerts -->
    <div v-if="error" class="rounded-xl border border-red-300 bg-red-100 p-4 text-red-600 text-sm flex justify-between">
      {{ error }}<button @click="error=''" class="text-red-500 ml-3">✕</button>
    </div>
    <div v-if="successMsg" class="rounded-xl border border-green-300 bg-green-100 p-4 text-green-700 text-sm flex justify-between">
      ✅ {{ successMsg }}<button @click="successMsg=''" class="text-green-600 ml-3">✕</button>
    </div>

    <!-- No schedule warning -->
    <div v-if="data && !data.schedule?.length"
         class="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-700 text-sm flex items-center gap-3">
      <span class="text-2xl">⚠️</span>
      <div class="flex-1">
        <p class="font-semibold">Day Schedule not set</p>
        <p class="text-amber-600 text-xs mt-0.5">Live monitor will work once the 15-day schedule is set up.</p>
      </div>
      <button @click="openScheduleSetup"
        class="px-4 py-2 rounded-lg border border-amber-300 text-amber-700 bg-white text-sm hover:bg-amber-50 transition shrink-0">
        📅 Setup
      </button>
    </div>
    ...
    <div v-if="data" class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="card p-4 col-span-2 md:col-span-2">
        <p class="text-gray-700 text-xs uppercase tracking-widest mb-1">Today — {{ fmtDate(data.today) }}</p>
        <div class="flex items-center gap-3">
          <span class="text-4xl">{{ data.todayIcon || '📍' }}</span>
          <div>
            <p class="text-gray-900 text-xl font-bold">{{ data.todayCity }}</p>
            <p class="text-amber-700 text-sm">{{ data.todayActivity || 'Day ' + (data.todayDayNumber ?? '—') }}</p>
          </div>
        </div>
      </div>
      <div class="card p-4 text-center">
        <p class="text-gray-700 text-xs uppercase tracking-widest mb-1">Total Pilgrims</p>
        <p class="text-3xl font-bold text-green-700">{{ data.totalPilgrims }}</p>
      </div>
      <div class="card p-4 text-center">
        <p class="text-gray-700 text-xs uppercase tracking-widest mb-1">Currently in Iraq</p>
        <p class="text-3xl font-bold text-amber-700">{{ data.activePilgrims }}</p>
      </div>
    </div>

    <!-- 15-day Schedule Timeline -->
    <div v-if="data && data.schedule?.length" class="card p-5">
      <h2 class="text-gray-900 font-semibold mb-4">📅 15-Day Journey Timeline</h2>
      <div class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-8 lg:grid-cols-15 gap-2">
        <div v-for="day in data.schedule" :key="day.dayNumber"
          ...
          <div class="font-bold" :class="isToday(day.date) ? 'text-amber-700' : 'text-gray-700'">Day {{ day.dayNumber }}</div>
          <div :class="isToday(day.date) ? 'text-amber-600' : 'text-gray-600'" class="truncate">{{ day.city }}</div>
          <div class="text-gray-600">{{ fmtShortDate(day.date) }}</div>
          <div v-if="isToday(day.date)" class="mt-1 text-amber-700 font-bold text-xs">TODAY</div>
          <div v-else-if="day.isKeyDay" class="mt-1 text-green-700 text-xs">KEY</div>
        </div>
      </div>
    </div>

    <!-- Pilgrims Table -->
    <div v-if="data" class="card overflow-hidden">
      <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between flex-wrap gap-3">
        <h2 class="text-gray-900 font-semibold">👥 Pilgrim Status</h2>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="text-gray-700 text-xs uppercase tracking-wider border-b border-gray-200 bg-gray-50">
          ...
          <tbody>
            <tr v-if="!filteredPilgrims.length">
              <td colspan="8" class="px-4 py-8 text-center text-gray-600">No pilgrims found</td>
            </tr>
            <tr v-for="(p, i) in filteredPilgrims" :key="p.id"
              class="border-b border-gray-100 hover:bg-gray-50 transition">
              <td class="px-4 py-3 text-gray-600">{{ i + 1 }}</td>
              <td class="px-4 py-3 text-gray-900 font-medium">{{ p.fullName }}</td>
              <td class="px-4 py-3 text-gray-600">{{ p.country || '—' }}</td>
              <td class="px-4 py-3 text-gray-700">{{ fmtDate(p.arrivalDate) }}</td>
              <td class="px-4 py-3 text-gray-700">{{ fmtDate(p.departureDate) }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3">
                <a v-if="p.whatsApp"
                  ...
                  class="text-green-600 hover:text-green-700 text-xs transition">📱 Message</a>
    ...
    <div v-if="showScheduleModal"
      ...
      <div class="w-full max-w-3xl rounded-2xl overflow-hidden shadow-2xl my-4"
        ...
        <div class="px-5 py-3 border-b border-slate-700">
          <p class="text-xs text-slate-400 mb-2">Quick presets (click any day to apply):</p>
          <div class="flex flex-wrap gap-2">
            <span v-for="p in CITY_PRESETS" :key="p.city+p.activity"
                  class="text-xs px-2 py-1 rounded-lg border border-slate-600 text-slate-300 cursor-help"
                  :title="p.activity">
              {{ p.icon }} {{ p.city }}
            </span>
          </div>
        ...
        <div class="p-5 space-y-2 max-h-[60vh] overflow-y-auto">
          <div v-for="day in scheduleForm" :key="day.dayNumber"
               :class="['rounded-xl border p-3', day.isKeyDay ? 'border-amber-700/50 bg-amber-950/20' : 'border-slate-700 bg-slate-800/30']">
            <div class="flex items-center gap-2 mb-2">
              <span class="text-slate-400 text-xs font-bold w-12 shrink-0">Day {{ day.dayNumber }}</span>
              <input v-model="day.date" type="date"
                class="bg-slate-800 border border-slate-600 rounded-lg px-2 py-1 text-slate-200 text-xs w-36 shrink-0" />
              <input v-model="day.icon" type="text"
                class="bg-slate-800 border border-slate-600 rounded-lg px-2 py-1 text-white text-sm w-12 text-center" placeholder="📍" />
            ...
            <div class="grid grid-cols-2 gap-2">
              <div class="flex gap-1">
                <input v-model="day.city" class="bg-slate-800 border border-slate-600 rounded-lg px-2 py-1.5 text-white text-xs flex-1" placeholder="City (e.g. Karbala)" />
                <!-- Quick preset buttons -->
                <div class="relative group">
                  <button class="text-slate-500 hover:text-slate-300 text-xs px-1.5 py-1.5 rounded border border-slate-700 hover:border-slate-500">▼</button>
                  <div class="absolute right-0 top-full mt-1 z-20 bg-slate-900 border border-slate-700 rounded-xl shadow-xl hidden group-focus-within:block group-hover:block w-48">
                    <button v-for="p in CITY_PRESETS" :key="p.city+p.icon"
                      @click="applyPreset(day, p)"
                      class="w-full text-left text-xs px-3 py-2 text-slate-300 hover:bg-slate-800 transition">
                      {{ p.icon }} {{ p.city }} — {{ p.activity.substring(0,20) }}
                    </button>
                  </div>
                </div>
              </div>
              <input v-model="day.activity" class="bg-slate-800 border border-slate-600 rounded-lg px-2 py-1.5 text-white text-xs" placeholder="Activity / Programme" />
            </div>
          </div>
        </div>
        ...
        <div class="px-5 py-4 border-t border-slate-700 flex gap-3">
          <button @click="saveSchedule" :disabled="savingSchedule"
            class="flex-1 py-2.5 rounded-xl font-semibold text-gray-900 disabled:opacity-50 text-sm transition"
            style="background:linear-gradient(135deg,#b88a00,#D4A800);">
            {{ savingSchedule ? 'Saving...' : '✅ Save Schedule' }}
          </button>
          <button @click="showScheduleModal=false" class="px-5 py-2.5 rounded-xl border border-slate-600 text-slate-400 hover:text-white text-sm">
            Cancel
          </button>
        </div>
      </div>

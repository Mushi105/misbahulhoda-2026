...
const statusColors = {
  Unassigned:     'text-gray-700 bg-gray-100',
  Pending:        'text-amber-700 bg-amber-100',
  DriverAssigned: 'text-blue-700 bg-blue-100',
  EnRoute:        'text-purple-700 bg-purple-100',
...
<template>
  <div class="p-6 space-y-6">
    ...
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold text-gray-900">✈️ Itinerary & Airport Transfers</h1>
      <p class="text-gray-700 text-sm mt-1">Manage pilgrim arrivals, departures and airport pickup/drop assignments</p>
    </div>

    <div v-if="success" class="bg-green-100 border border-green-300 text-green-700 rounded-lg px-4 py-3 flex justify-between">
      {{ success }}<button @click="success=''" class="text-green-600">✕</button>
    </div>
    <div v-if="error" class="bg-red-100 border border-red-300 text-red-600 rounded-lg px-4 py-3 flex justify-between">
      {{ error }}<button @click="error=''" class="text-red-600">✕</button>
    </div>

    <!-- Tabs + Send Reminders -->
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div class="flex gap-1 bg-gray-100 rounded-xl p-1 border border-gray-200">
        <button v-for="[t,label] in [['today','📅 Today'],['upcoming','📆 Upcoming'],['arrivals','🛬 Arrivals'],['departures','🛫 Departures']]"
          :key="t" @click="switchTab(t)"
          :class="['px-4 py-2 rounded-lg text-sm font-medium transition-all', tab === t ? 'bg-amber-600 text-white' : 'text-gray-700 hover:text-gray-900']">
          {{ label }}
        </button>
      </div>
    ...
    <div v-if="loading" class="text-center py-20 text-gray-700">Loading...</div>

    <!-- ── TODAY ─────────────────────────────────────────────────────────────── -->
    <template v-else-if="tab === 'today' && today">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-green-700">{{ today.arrivingCount }}</div>
          <div class="text-gray-600 text-xs mt-1">🛬 Arriving Today</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-blue-600">{{ today.departingCount }}</div>
          <div class="text-gray-600 text-xs mt-1">🛫 Departing Today</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-red-500">{{ today.pendingPickups }}</div>
          <div class="text-gray-600 text-xs mt-1">⚠️ Pickups Unassigned</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-orange-600">{{ today.pendingDropoffs }}</div>
          <div class="text-gray-600 text-xs mt-1">⚠️ Dropoffs Unassigned</div>
        </div>
      </div>

      <div class="card">
        <h3 class="text-gray-900 font-semibold mb-4">🛬 Today's Arrivals
          <span class="text-gray-600 font-normal text-sm">({{ today.arrivals?.length || 0 }})</span>
        </h3>
        ...
        <div v-else class="text-center py-8 text-gray-700 text-sm">No arrivals today.</div>
      </div>

      <div class="card">
        <h3 class="text-gray-900 font-semibold mb-4">🛫 Today's Departures
          <span class="text-gray-600 font-normal text-sm">({{ today.departures?.length || 0 }})</span>
        </h3>
        ...
        <div v-else class="text-center py-8 text-gray-700 text-sm">No departures today.</div>
      </div>
    </template>
    ...
    <template v-else-if="tab === 'upcoming'">
      ...
      <div class="flex items-center gap-4 flex-wrap">
        <div class="flex items-center gap-2">
          <label class="text-gray-700 text-sm">Show next</label>
        ...
        <div class="flex flex-col gap-1">
          <p class="text-gray-700 text-sm">📅 <strong class="text-gray-700">8:00 AM UTC</strong> — 2-day advance reminders sent to all travellers.</p>
          <p class="text-gray-700 text-sm">⏱️ <strong class="text-gray-700">Every 30 min</strong> — 5-hour-before reminder fires when flight is 5h away.</p>
          <p class="text-gray-700 text-sm">🌙 <strong class="text-red-600">8:00 PM UTC</strong> — If driver not assigned for tomorrow: pilgrim notified, admin alerted. <em class="text-gray-600">Auto safety net.</em></p>
        </div>
      </div>

      <div v-if="!upcoming?.dates?.length" class="card text-center py-16 text-gray-700">
      ...
      <div v-for="day in upcoming?.dates" :key="day.date" class="card space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="text-gray-900 font-bold text-base">📅 {{ day.dayLabel }}</h3>
          <div class="flex gap-3 text-xs">
            <span v-if="day.arrivingCount" class="text-green-700 bg-green-100 px-2 py-0.5 rounded-full">
              🛬 {{ day.arrivingCount }} arriving
            </span>
            <span v-if="day.departingCount" class="text-blue-700 bg-blue-100 px-2 py-0.5 rounded-full">
        ...
        <div v-if="day.arrivals?.length">
          <p class="text-green-700 text-xs font-semibold uppercase tracking-wider mb-2">🛬 Arrivals</p>
        ...
        <div v-if="day.departures?.length">
          <p class="text-blue-700 text-xs font-semibold uppercase tracking-wider mb-2">🛫 Departures</p>
    ...
    <template v-else-if="tab === 'arrivals'">
      ...
      <div class="card">
        <h3 class="text-gray-900 font-semibold mb-4">🛬 Arrivals on {{ fmtDate(arrivalDate) }}
          <span class="text-gray-600 font-normal text-sm">({{ arrivals.length }})</span>
        </h3>
    ...
    <template v-else-if="tab === 'departures'">
      ...
      <div class="card">
        <h3 class="text-gray-900 font-semibold mb-4">🛫 Departures on {{ fmtDate(departureDate) }}
          <span class="text-gray-600 font-normal text-sm">({{ departures.length }})</span>
        </h3>
    ...
    <Teleport to="body">
      <div v-if="showAssign" class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
        <div class="rounded-2xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto" style="background:#0f172a; border:1px solid rgba(180,83,9,0.35);">
          ...
          <div class="flex gap-3 mt-5">
            ...
            <button @click="showAssign = false" class="px-4 py-2 text-slate-400 hover:text-white text-sm border border-slate-600 rounded-xl">Cancel</button>
          </div>
        </div>
      </div>
    </Teleport>
      ...
      template: `
        <div class="overflow-x-auto">
          <table class="w-full text-xs">
            <thead>
              <tr class="text-gray-700 border-b border-gray-200 uppercase">
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.pilgrimId" class="border-b border-gray-100 hover:bg-gray-50">
                <td class="py-2 pr-3">
                  <p class="text-gray-900 font-semibold">{{ row.fullName }}</p>
                  <p class="text-gray-700">{{ row.country }}</p>
                  <a v-if="wa(row.whatsApp || row.phone)" :href="wa(row.whatsApp || row.phone)"
                    target="_blank" class="text-green-600 hover:underline">💬 WA</a>
                </td>
                <td class="py-2 pr-3">
                  <p class="text-gray-900">{{ row.flightNumber || '—' }}</p>
                  <p class="text-gray-700">{{ row.airport || 'Airport not set' }}</p>
                </td>
                <td class="py-2 pr-3 text-gray-900">{{ row.time || '—' }}</td>
                <td class="py-2 pr-3">
                  <span class="text-amber-700 font-semibold">{{ row.familyCount }}</span>
                </td>
                <td class="py-2 pr-3">
                  <span :class="['px-2 py-0.5 rounded-full font-medium', statusColors[row.transferStatus] || 'text-gray-700 bg-gray-100']">
                    {{ statusIcons[row.transferStatus] }} {{ row.transferStatus }}
                  </span>
                  <p v-if="row.transfer?.driverName" class="text-gray-700 mt-0.5">
                    🚗 {{ row.transfer.driverName }}
                  </p>
                </td>
                <td class="py-2">
                  <button @click="$emit('assign', row)"
                    class="px-2 py-1 rounded bg-amber-600 hover:bg-amber-500 text-white whitespace-nowrap">
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.pilgrimId" class="border-b border-gray-100 hover:bg-gray-50">
                <td class="py-3 pr-3">
                  <p class="text-gray-900 font-semibold">{{ row.fullName }}</p>
                  <p class="text-gray-600 text-xs">{{ row.country }}</p>
                  <a v-if="wa(row.whatsApp || row.phone)" :href="wa(row.whatsApp || row.phone)"
                    target="_blank" class="text-xs text-green-600 hover:underline">💬 WhatsApp</a>
                </td>
                <td class="py-3 pr-3">
                  <p class="text-gray-900">{{ row.flightNumber || '—' }}</p>
                  <p class="text-gray-600 text-xs">{{ row.airport || 'Airport not set' }}</p>
                </td>
                <td class="py-3 pr-3">
                  <p class="text-gray-900">{{ fmtDate(row.date) }}</p>
                  <p class="text-gray-600 text-xs">{{ row.time || 'Time not set' }}</p>
                </td>
                <td class="py-3 pr-3">
                  <span class="text-amber-700 font-semibold">{{ row.familyCount }}</span>
                  <span class="text-gray-600 text-xs"> member{{ row.familyCount !== 1 ? 's' : '' }}</span>
                </td>
                <td class="py-3 pr-3">
                  <div class="flex items-center gap-2">
                    <span :class="['text-xs px-2 py-0.5 rounded-full font-medium', statusColors[row.transferStatus] || 'text-gray-700 bg-gray-100']">
                      {{ statusIcons[row.transferStatus] }} {{ row.transferStatus }}
                    </span>
                    <select v-if="row.transferId"
                      :disabled="updatingId === row.transferId"
                      @change="$emit('status', row.transferId, $event.target.value)"
                      class="text-xs bg-white border border-gray-300 text-gray-700 rounded px-1 py-0.5">
                      <option v-for="s in statusOptions" :key="s.value" :value="s.value">{{ s.label }}</option>
                    </select>
                  </div>
                  <div v-if="row.transfer?.scheduledTime" class="text-xs text-gray-600 mt-1">
                    🕐 {{ fmtTime(row.transfer.scheduledTime) }}
                  </div>
                </td>
                <td class="py-3 pr-3">
                  <div v-if="row.transfer?.driverName">
                    <p class="text-gray-900 text-xs font-semibold">{{ row.transfer.driverName }}</p>
                    <p class="text-gray-600 text-xs">{{ row.transfer.vehicleType }} · {{ row.transfer.vehicleNumber || '—' }}</p>
                    <a v-if="wa(row.transfer.driverPhone)" :href="wa(row.transfer.driverPhone)"
                      target="_blank" class="text-xs text-green-600 hover:underline">📞 {{ row.transfer.driverPhone }}</a>
                    <p v-if="row.transfer.meetingPoint" class="text-gray-600 text-xs mt-0.5">📍 {{ row.transfer.meetingPoint }}</p>
                  </div>
                  <span v-else class="text-gray-600 text-xs italic">Not assigned</span>
                </td>
                <td class="py-3">
                  <div class="flex flex-col gap-1">
                    <button @click="$emit('assign', row, type)"
                      class="text-xs px-2 py-1 rounded bg-amber-600 hover:bg-amber-500 text-white whitespace-nowrap">
                      {{ row.transfer ? '✏️ Edit' : '+ Assign' }}
                    </button>
                    <button v-if="row.transferId" @click="$emit('remove', row.transferId)"
                      class="text-xs px-2 py-1 rounded border border-red-300 text-red-600 hover:bg-red-50 whitespace-nowrap">
                      Remove
                    </button>
                  </div>
                </td>

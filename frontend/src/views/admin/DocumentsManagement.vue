  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-700 text-sm mt-0.5">Upload tour guides, manuals and notices for pilgrims</p>
      </div>
      <button @click="openUpload"
              class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-900"
    ...
    <!-- Loading -->
    <div v-if="loading" class="card text-center py-12 text-gray-700">Loading documents...</div>

    <!-- Empty -->
    <div v-else-if="!docs.length" class="card text-center py-16">
      <div class="text-5xl mb-4">📂</div>
      <p class="text-gray-900 font-bold text-lg">No documents uploaded yet</p>
      <p class="text-gray-700 text-sm mt-2 mb-6">Upload tour guides, manuals, and notices for pilgrims.</p>
      <button @click="openUpload"
          ...
          <div v-for="doc in catDocs" :key="doc.id"
            ...
            <div class="flex items-start gap-3">
              <div class="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                   :style="doc.contentType === 'application/pdf' ? 'background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3)' : 'background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.3)'">
                {{ doc.contentType === 'application/pdf' ? '📕' : '🖼️' }}
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-gray-900 font-semibold text-sm leading-tight">{{ doc.title }}</p>
                <p v-if="doc.description" class="text-gray-600 text-xs mt-0.5 line-clamp-2">{{ doc.description }}</p>
                <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span class="text-xs text-gray-600">{{ doc.sizeLabel }}</span>
                  <span class="text-gray-300">·</span>
                  <span class="text-xs text-gray-600">{{ new Date(doc.createdAt).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' }) }}</span>
                  <span class="text-gray-300">·</span>
                  <span class="text-xs text-gray-600">{{ doc.uploadedBy }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-200">
              <button @click="viewInBrowser(doc)"
                      class="flex-1 text-xs px-3 py-1.5 rounded-lg border border-blue-300 text-blue-600 hover:bg-blue-50 transition-colors font-medium">
                👁 View
              </button>
              <button @click="download(doc)"
                      class="flex-1 text-xs px-3 py-1.5 rounded-lg border border-emerald-300 text-amber-700 hover:bg-amber-50 transition-colors font-medium">
                ⬇ Download
              </button>
              <button @click="deleteDoc(doc)" :disabled="deleting === doc.id"
                      class="text-xs px-3 py-1.5 rounded-lg border border-red-300 text-red-600 hover:bg-red-50 transition-colors font-medium disabled:opacity-50">
                🗑
              </button>
            </div>
          </div>
      ...
      <div v-if="showUpload"
        ...
        <div class="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl"
          ...
          <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            ...
            <button @click="showUpload = false" class="text-gray-300 hover:text-white text-2xl">✕</button>
          </div>
          ...
          <div class="p-6 space-y-4">
            ...
            <div @dragover.prevent="dragOver = true"
                 ...
                 :class="['border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all',
                   dragOver ? 'border-amber-500 bg-amber-50' : uploadFile ? 'border-emerald-400 bg-emerald-50' : 'border-gray-300 bg-gray-50 hover:border-amber-400']">
              ...
              <div v-if="uploadFile">
                ...
                <p class="text-gray-300 text-xs mt-1">{{ fileSizeLabel(uploadFile) }}</p>
                <p class="text-amber-400 text-xs mt-2">Click to change file</p>
              </div>
              <div v-else>
                <div class="text-4xl mb-3">📂</div>
                <p class="text-white font-medium">Drop file here or click to browse</p>
                <p class="text-gray-600 text-xs mt-1">PDF, JPG, PNG — max 20 MB</p>
              </div>
            </div>

...
const addHotelForm    = ref({ name: '', city: '', address: '', phoneNumber: '', latitude: '', longitude: '', nearHaram: '', haramDistanceText: '' })
...
// ── Hotel Setup actions ──────────────────────────────────
function openAddHotel() { addHotelForm.value = { name: '', city: '', address: '', phoneNumber: '', latitude: '', longitude: '', nearHaram: '', haramDistanceText: '' }; showAddHotel.value = true }
            ...
            <div class="rounded-xl border border-emerald-900/30 p-4 space-y-3" style="background:rgba(16,185,129,0.04);">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input v-model="addHotelForm.latitude" class="input w-full" placeholder="e.g. 32.6174" type="number" step="0.0001" />
                </div>
                <div>
                  <label class="form-label">Longitude</label>
                  <input v-model="addHotelForm.longitude" class="input w-full" placeholder="e.g. 44.0190" type="number" step="0.0001" />
                </div>
              </div>
              <a v-if="addHotelForm.latitude && addHotelForm.longitude"
                 :href="`https://www.google.com/maps?q=${addHotelForm.latitude},${addHotelForm.longitude}`"
                 target="_blank" rel="noopener"
                 class="inline-flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300">
                🗺️ Check on map
              </a>
              <div>
                <label class="form-label">Distance from Haram (text)</label>
                <input v-model="addHotelForm.haramDistanceText" class="input w-full"
                       placeholder="e.g. ~400m from Imam Hussain Haram" />
              </div>
              <div>
                <label class="form-label">Walking time / Note</label>
                <input v-model="addHotelForm.nearHaram" class="input w-full"
                       placeholder="e.g. 5 min walk to Haram" />
              </div>
            </div>

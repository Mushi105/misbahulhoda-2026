...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-2xl font-bold text-gray-900">Tour Management</h2>
        <p class="text-gray-700 text-sm mt-1">Manage tours, packages & pilgrim history</p>
      </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      <div class="card text-center">
        <p class="text-3xl font-bold text-amber-600">{{ tours.length }}</p>
        <p class="text-gray-700 text-sm">Total Tours</p>
      </div>
      <div class="card text-center">
        <p class="text-3xl font-bold text-green-700">{{ openTours }}</p>
        <p class="text-gray-700 text-sm">Open for Apply</p>
      </div>
      <div class="card text-center">
        <p class="text-3xl font-bold text-blue-600">{{ activeTours }}</p>
        <p class="text-gray-700 text-sm">Active Tours</p>
      </div>
      <div class="card text-center">
        <p class="text-3xl font-bold text-amber-700">{{ totalPilgrims }}</p>
        <p class="text-gray-700 text-sm">Total Pilgrims</p>
      </div>
    </div>
    ...
    <div class="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
      <button v-for="tab in ['tours','feedback']" :key="tab" @click="activeTab = tab"
        :class="['px-5 py-2 rounded-lg text-sm font-medium capitalize transition-colors',
          activeTab === tab ? 'bg-amber-600 text-white' : 'text-gray-700 hover:text-gray-900']">
        {{ tab }}
      </button>
    </div>
    ...
    <div v-if="activeTab === 'tours'">
      <div v-if="loading" class="text-center py-12 text-gray-700">Loading tours...</div>
      <div v-else-if="!tours.length" class="text-center py-12">
        <p class="text-gray-700 mb-4">No tours created yet</p>
          ...
          <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
            <div>
              <div class="flex items-center gap-3 flex-wrap">
                ...
                <span v-if="tour.isOpen" class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">
                  Open
                </span>
              </div>
              ...
              <p v-if="tour.description" class="text-gray-700 text-sm mt-1">{{ tour.description }}</p>
            </div>
          ...
          <div v-if="tour.packages?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <div v-for="pkg in tour.packages" :key="pkg.id"
              ...
              <div class="flex items-start justify-between mb-3">
                <div>
                  <p class="text-gray-900 font-semibold text-sm">{{ pkg.packageName }}</p>
                  <span v-if="!pkg.isActive" class="text-xs text-gray-600 italic">inactive</span>
                </div>
                <div class="flex gap-1">
                  <button @click="openEditPackage(tour, pkg)" class="text-gray-600 hover:text-gray-900 p-1">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
                  </button>
                  <button @click="deletePackage(pkg.id)" class="text-gray-600 hover:text-red-500 p-1">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Pricing block -->
              <div class="rounded-lg p-3 mb-3 bg-amber-50 border border-amber-200">
                <div class="flex items-end justify-between">
                  <div>
                    <p class="text-xs text-gray-600 mb-0.5">Per person</p>
                    <p class="text-amber-700 font-bold text-xl leading-none">
                      {{ Number(pkg.price).toLocaleString() }}
                      <span class="text-sm font-normal text-amber-600">{{ pkg.currency }}</span>
                    </p>
                  </div>
                  <div class="text-right">
                    <p class="text-xs text-gray-600 mb-0.5">Per day</p>
                    <p class="text-gray-700 font-medium text-sm">
                      {{ pkg.durationDays > 0 ? Math.round(pkg.price / pkg.durationDays).toLocaleString() : '—' }}
                      <span class="text-xs text-gray-600">{{ pkg.currency }}/day</span>
                    </p>
                  </div>
                </div>
                <div class="border-t border-amber-200 mt-2 pt-2">
                  <div class="flex items-center justify-between text-xs">
                    <span class="text-gray-700">Total capacity revenue</span>
                    <span class="text-green-700 font-medium">
                      {{ (Number(pkg.price) * pkg.maxPilgrims).toLocaleString() }} {{ pkg.currency }}
                    </span>
                  </div>
                  <div class="flex items-center justify-between text-xs mt-1">
                    <span class="text-gray-700">Enrolled / Max</span>
                    <span class="text-gray-700 font-medium">
                      {{ pkg.pilgrimsCount || 0 }} / {{ pkg.maxPilgrims }}
                      <span v-if="pkg.maxPilgrims > 0" class="text-gray-600">
              ...
              <div class="text-xs text-gray-600 space-y-1.5">
                ...
                <p v-if="pkg.routeDetail" class="text-gray-700 italic truncate" :title="pkg.routeDetail">
                  {{ pkg.routeDetail }}
                </p>
                <p>{{ pkg.durationDays }} days · {{ pkg.maxPilgrims }} seats total</p>
                <p v-if="pkg.hotelInfo" class="text-gray-600 truncate">{{ pkg.hotelInfo }}</p>
                <div class="flex flex-wrap gap-1 mt-1">
                  <span v-if="pkg.includesAirportTransfer" class="px-1.5 py-0.5 rounded bg-blue-100 text-blue-700">✈ Transfer</span>
                  <span v-if="pkg.includesBreakfast" class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">🍳 B</span>
                  <span v-if="pkg.includesLunch" class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">🍱 L</span>
                  <span v-if="pkg.includesDinner" class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">🍽 D</span>
                </div>
              </div>
            </div>
          </div>
          <p v-else class="text-gray-600 text-sm italic">No packages yet — add one to open applications</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'feedback'">
      <div v-if="!selectedTour" class="text-center py-12">
        <p class="text-gray-700 mb-2">Select a tour from the Tours tab to view feedback</p>
        <button @click="activeTab = 'tours'" class="btn-ghost text-sm">Go to Tours</button>
      </div>
      <div v-else>
        <div class="flex items-center gap-3 mb-6">
          <button @click="activeTab = 'tours'" class="text-gray-600 hover:text-gray-900">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
          </button>
          <h3 class="text-gray-900 font-bold">Feedback — {{ selectedTour.tourName }}</h3>
        </div>
        <div v-if="feedbackLoading" class="text-center py-12 text-gray-700">Loading feedback...</div>
        <div v-else-if="!feedback || !feedback.totalResponses" class="text-center py-12">
          <p class="text-gray-700">No feedback received for this tour yet</p>
          <button @click="sendFeedbackEmails(selectedTour.id)" class="btn-gold mt-4 text-sm">Send Feedback Request Emails</button>
        </div>
        <div v-else class="space-y-6">
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div class="card text-center">
              <p class="text-3xl font-bold text-amber-600">{{ feedback.totalResponses }}</p>
              <p class="text-gray-600 text-xs">Responses</p>
            </div>
            <div class="card text-center">
              <p class="text-3xl font-bold text-amber-700">{{ feedback.averageOverall?.toFixed(1) }}/5</p>
              <p class="text-gray-600 text-xs">Overall</p>
            </div>
            <div class="card text-center">
              <p class="text-3xl font-bold text-blue-600">{{ feedback.averageHotel?.toFixed(1) }}/5</p>
              <p class="text-gray-600 text-xs">Hotel</p>
            </div>
            <div class="card text-center">
              <p class="text-3xl font-bold text-green-700">{{ feedback.averageTransport?.toFixed(1) }}/5</p>
              <p class="text-gray-600 text-xs">Transport</p>
            </div>
            <div class="card text-center">
              <p class="text-3xl font-bold text-purple-600">{{ feedback.recommendPercent?.toFixed(0) }}%</p>
              <p class="text-gray-600 text-xs">Would Recommend</p>
            </div>
          </div>
          <div v-if="feedback.responses?.length" class="space-y-3">
            <h4 class="text-gray-900 font-medium">Individual Responses</h4>
            <div v-for="r in feedback.responses" :key="r.id"
              class="card text-sm">
              <div class="flex items-center justify-between mb-2">
                <div class="flex gap-3 text-gray-700">
                  <span>Overall: <span class="text-amber-700 font-bold">{{ r.overallRating }}/5</span></span>
                  <span v-if="r.hotelRating">Hotel: {{ r.hotelRating }}/5</span>
                  <span v-if="r.transportRating">Transport: {{ r.transportRating }}/5</span>
                </div>
                <div class="flex gap-2 text-xs">
                  <span v-if="r.wouldRecommend" class="text-green-700">✓ Recommends</span>
                  <span v-if="r.wouldReturn" class="text-blue-600">✓ Returns</span>
                </div>
              </div>
              <p v-if="r.comment" class="text-gray-700">"{{ r.comment }}"</p>
              <p v-if="r.suggestions" class="text-gray-700 mt-1">Suggestion: {{ r.suggestions }}</p>
              <p v-if="r.preferredPackageNextYear" class="text-amber-600 mt-1">Next year: {{ r.preferredPackageNextYear }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="showTourModal" class="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div class="card w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h3 class="text-gray-900 font-bold text-lg mb-4">{{ editingTour ? 'Edit Tour' : 'Create New Tour' }}</h3>
        <div class="space-y-3">
          <div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div class="flex items-center gap-3 pt-5">
              ...
              <label for="isOpen" class="text-gray-600 text-sm">Open for Applications</label>
            </div>
          </div>
          <div>
        ...
        <div v-if="tourFormError" class="mt-4 p-3 rounded-lg bg-red-100 border border-red-300 text-red-600 text-sm">
          {{ tourFormError }}
        </div>
    ...
    <div v-if="showPackageModal" class="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div class="card w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <h3 class="text-gray-900 font-bold text-lg mb-1">{{ editingPackage ? 'Edit Package' : 'Add Package' }}</h3>
        <p class="text-gray-700 text-sm mb-4">{{ selectedTour?.tourName }}</p>
        <div class="space-y-3">
          <div>
          ...
          <div class="rounded-lg p-3 space-y-3 bg-gray-50 border border-gray-200">
            <p class="text-gray-600 text-sm font-medium">Trip Type</p>
            ...
            <div>
              ...
              <div class="flex gap-2">
                <label :class="['flex-1 flex items-center justify-center py-2 rounded-lg border cursor-pointer text-sm font-medium transition-colors',
                  packageForm.tripDuration == 1 ? 'border-sky-400 bg-sky-100 text-sky-700' : 'border-gray-300 text-gray-700 hover:border-gray-400']">
                  <input type="radio" v-model.number="packageForm.tripDuration" :value="1" class="hidden" />
                  Short Trip
                </label>
                <label :class="['flex-1 flex items-center justify-center py-2 rounded-lg border cursor-pointer text-sm font-medium transition-colors',
                  packageForm.tripDuration == 2 ? 'border-violet-400 bg-violet-100 text-violet-700' : 'border-gray-300 text-gray-700 hover:border-gray-400']">
            ...
            <div>
              <label class="label">Destinations <span class="text-gray-600 font-normal">(select all that apply)</span></label>
                ...
                <button v-for="d in DESTINATIONS" :key="d" type="button"
                  ...
                  :class="['px-3 py-1.5 rounded-full border text-sm font-medium transition-colors',
                    selectedDests.includes(d)
                      ...
                      : 'border-gray-300 text-gray-700 hover:border-gray-400 hover:text-gray-700']">
                  {{ d }}
                </button>
              </div>
              <p v-if="!selectedDests.length" class="text-gray-600 text-xs mt-1">Select at least one destination</p>
            </div>

            <!-- Route detail -->
            <div>
              <label class="label">Route Detail <span class="text-gray-600 font-normal">(optional — flight path)</span></label>

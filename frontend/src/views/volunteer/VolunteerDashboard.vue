  ...
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between">
      <div>
        ...
        <p class="text-gray-700 text-sm">Welcome, {{ auth.user?.fullName }}</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <template v-else-if="volunteerInfo?.isCheckedIn">
          <span class="text-xs bg-emerald-900/50 border border-emerald-700 text-amber-700 rounded-lg px-3 py-1.5">● On Duty</span>
    ...
    <template v-else>
      ...
      <div v-if="!volunteerInfo" class="card text-center py-10">
        ...
        <h2 class="text-gray-900 font-semibold text-lg">Not Registered as Volunteer</h2>
        <p class="text-gray-700 text-sm mt-2">Contact an admin to register you as a volunteer.</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="card text-center py-4">
            <div class="text-3xl font-bold text-gray-900">{{ tasks.length }}</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="card col-span-1 space-y-4">
            <h2 class="text-base font-semibold text-gray-900 border-b border-gray-200 pb-2">My Status</h2>
            <div class="space-y-3 text-sm">
              ...
              <div v-if="volunteerInfo.zone" class="flex justify-between">
                ...
                <span class="text-amber-700 font-medium">{{ volunteerInfo.zone.name }}</span>
              </div>
              <div v-if="volunteerInfo.assignedArea" class="flex justify-between">
                <span class="text-gray-700">Area</span>
                <span class="text-gray-900">{{ volunteerInfo.assignedArea }}</span>
              </div>
              <div v-if="volunteerInfo.shiftStart" class="flex justify-between">
                <span class="text-gray-700">Shift</span>
                <span :class="getShiftStatus() === 'active' ? 'text-green-600' : 'text-gray-600'">
                  {{ volunteerInfo.shiftStart }} – {{ volunteerInfo.shiftEnd }}
                  <span v-if="getShiftStatus() === 'active'" class="text-xs ml-1">(Active)</span>
                </span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-700">Tasks Done</span>
                <span class="text-amber-700 font-semibold">{{ volunteerInfo.totalTasksCompleted }}</span>
              </div>
            </div>
            <!-- Change Status -->
            <div class="pt-2 border-t border-gray-200">
              <p class="text-xs text-gray-600 mb-2">Update your status:</p>
                ...
                <button v-for="(s, key) in volunteerStatusConfig" :key="key"
                  ...
                  :class="['text-xs py-1.5 rounded-lg border transition-colors',
                    volunteerInfo.status === key
                      ? 'border-primary-600 bg-primary-900/40 text-amber-700'
          ...
          <div class="card col-span-2">
            <h2 class="text-base font-semibold text-gray-900 mb-3">👥 My Assigned Pilgrims ({{ volunteerInfo.assignedPilgrims?.length || 0 }})</h2>
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div v-for="p in volunteerInfo.assignedPilgrims" :key="p.id"
                class="bg-gray-100 rounded-lg p-3 flex items-start gap-3">
                <div class="w-9 h-9 rounded-full bg-amber-700 flex items-center justify-center text-gray-900 font-bold text-sm shrink-0">
                  {{ p.fullName?.charAt(0) || '?' }}
                </div>
                <div class="text-sm">
                  <p class="text-gray-900 font-medium">{{ p.fullName }}</p>
                  <p class="text-gray-600 text-xs">{{ p.country || '' }} · {{ p.passportNumber || '—' }}</p>
                  <p v-if="p.notes" class="text-gray-600 text-xs mt-0.5">{{ p.notes }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- Tasks List -->
          <div class="card col-span-2">
            <h2 class="text-base font-semibold text-gray-900 mb-4">My Tasks</h2>
                ...
                <div class="flex items-start justify-between gap-3">
                  ...
                  <div class="flex flex-col items-end gap-2 shrink-0">
                    ...
                    <button v-if="task.status !== 'Completed' && task.status !== 'Cancelled'"
                      ...
                      class="text-xs bg-emerald-900/50 hover:bg-emerald-800 text-amber-700 border border-emerald-700 rounded px-2 py-1 transition-colors">
                      ✓ Done
                    </button>
                  </div>
                </div>
                <div v-if="task.notes" class="mt-2 text-xs text-gray-700 bg-gray-100 rounded p-2">
                  Note: {{ task.notes }}
                </div>
              </div>
            </div>

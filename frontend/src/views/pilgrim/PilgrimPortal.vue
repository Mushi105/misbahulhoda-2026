...
<template>
  <div class="min-h-screen p-4 md:p-6 space-y-6">
    <div class="flex items-start justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">My Pilgrim Portal</h1>
        <p class="text-gray-700 text-sm mt-1">Welcome, {{ auth.user?.fullName }}</p>
      </div>
      <div class="text-gold-400 font-arabic text-lg hidden sm:block">لَبَّيْكَ اللَّهُمَّ لَبَّيْكَ</div>
    </div>

    <div v-if="success" class="bg-green-100 border border-green-300 text-green-700 rounded-lg px-4 py-3 flex justify-between">
      {{ success }}<button @click="success=''" class="text-green-500">✕</button>
    </div>
    <div v-if="error" class="bg-red-100 border border-red-300 text-red-600 rounded-lg px-4 py-3 flex justify-between">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <div
        ...
        <div class="flex items-center gap-4 px-5 py-4">
          ...
          <div class="flex-1">
            <p class="text-xs font-bold uppercase tracking-wider text-amber-700 mb-0.5">Misbah ul Hoda — Arbaeen 2026</p>
            <h2 class="text-lg font-bold text-gray-900 leading-tight">Complete Tour Guide</h2>
            <p class="text-gray-700 text-sm mt-0.5">14-day schedule · Hotels · Scholars · Arbaeen Walk · Holy Sites</p>
          </div>
          <div class="shrink-0 flex items-center gap-2 bg-amber-600 group-hover:bg-amber-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
        ...
        <div class="flex gap-3 px-5 pb-3 flex-wrap">
          ...
          <span class="text-xs bg-dark-900/60 text-amber-700 border border-primary-900 px-2.5 py-1 rounded-full">Kadhimiyyah · Najaf · Karbala</span>
      ...
      <div
        ...
        <div class="flex items-center gap-4 px-5 py-4">
          ...
          <div class="flex-1">
            ...
            <h2 class="text-lg font-bold text-gray-900 leading-tight">Documents & Guides</h2>
            <p class="text-gray-700 text-sm mt-0.5">Manuals, notices, maps and official documents</p>
          </div>
    ...
    <div v-if="loading" class="flex justify-center py-20 text-gray-700">Loading your profile...</div>

    <!-- Profile Form (create OR edit) -->
    <div v-else-if="showProfileForm" class="max-w-2xl">
      <div class="card">
        <div class="flex items-center justify-between mb-1">
          <h2 class="text-lg font-semibold text-gray-900">{{ profile ? 'Edit Your Profile' : 'Complete Your Pilgrim Profile' }}</h2>
          <button v-if="profile" @click="showProfileForm = false" class="text-gray-700 hover:text-gray-900 text-xl leading-none">✕</button>
        </div>
        <p class="text-gray-700 text-sm mb-6">Fill in your travel details to register for Arbaeen 2026.</p>
    ...
    <template v-else-if="profile && !showProfileForm">
      ...
      <div :class="['rounded-xl border overflow-hidden',
        ...
        style="background:#ffffff; backdrop-filter:blur(8px);">
        ...
        <div class="p-5">
          ...
          <div class="flex items-center gap-4 mb-3">
            ...
            <div>
              <p class="text-xs font-bold uppercase tracking-wider mb-0.5"
                :class="profile.status === 'Approved' ? 'text-amber-700' : profile.status === 'Rejected' ? 'text-red-400' : profile.status === 'UnderReview' ? 'text-blue-400' : 'text-amber-400'">
                Application Status
              </p>
              <p class="text-gray-900 font-bold text-lg leading-tight">{{ statusConfig[profile.status]?.label }}</p>
            </div>
          </div>

          <!-- Pending -->
          <template v-if="profile.status === 'Pending'">
            <p class="text-gray-600 text-sm mb-3">Your application has been submitted successfully. Our team will begin reviewing it shortly.</p>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div class="rounded-lg px-3 py-2.5 text-xs" style="background:rgba(212,168,0,0.08); border:1px solid rgba(212,168,0,0.15);">
                <p class="text-amber-700 font-semibold mb-0.5">✅ Submitted</p>
          ...
          <template v-else-if="profile.status === 'UnderReview'">
            <p class="text-gray-600 text-sm mb-3">Our team is currently reviewing your application. Please ensure all your profile details and documents are complete.</p>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              <div class="rounded-lg px-3 py-2.5 text-xs" style="background:rgba(212,168,0,0.08); border:1px solid rgba(212,168,0,0.15);">
                <p class="text-amber-700 font-semibold mb-0.5">📋 Action Needed</p>
          ...
          <template v-else-if="profile.status === 'Approved'">
            <div class="flex items-center gap-2 mb-3">
              <span class="font-arabic text-amber-700 text-base">مُبَارَک</span>
              <p class="text-amber-600 text-sm font-medium">Your application has been approved!</p>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div class="rounded-lg px-3 py-2.5 text-xs" style="background:rgba(212,168,0,0.10); border:1px solid rgba(212,168,0,0.25);">
                <p class="text-amber-700 font-semibold mb-0.5">🏨 Room Allocation</p>
                <p class="text-slate-400">{{ profile.room ? `Room ${profile.room.roomNumber} assigned` : 'Being arranged by admin' }}</p>
              </div>
              <div class="rounded-lg px-3 py-2.5 text-xs" style="background:rgba(212,168,0,0.08); border:1px solid rgba(212,168,0,0.15);">
                <p class="text-amber-700 font-semibold mb-0.5">🚌 Bus Allocation</p>
                <p class="text-slate-400">{{ profile.bus ? `Bus ${profile.bus.busNumber} assigned` : 'Being arranged by admin' }}</p>
              </div>
              <div class="rounded-lg px-3 py-2.5 text-xs" style="background:rgba(212,168,0,0.08); border:1px solid rgba(212,168,0,0.15);">
                <p class="text-amber-700 font-semibold mb-0.5">📖 Tour Guide</p>
          ...
          <template v-else-if="profile.status === 'Rejected'">
            <p class="text-gray-600 text-sm mb-3">Unfortunately your application was not approved at this time. See the reason below and contact us if you have questions.</p>
            ...
            <p class="text-gray-600 text-xs">If you believe this is an error, please contact us on WhatsApp or email for assistance.</p>
          </template>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-amber-700">{{ (profile.familyMembers?.length ?? 0) + 1 }}</div>
          <div class="text-gray-600 text-xs mt-1">Total Members</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-blue-400">{{ maleCount }}</div>
          <div class="text-gray-600 text-xs mt-1">Male (مرد)</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-pink-400">{{ femaleCount }}</div>
          <div class="text-gray-600 text-xs mt-1">Female (عورت)</div>
        </div>
        <div class="card text-center py-4">
          <div class="text-3xl font-bold text-gold-400">{{ profile.familyMembers?.filter(m => m.isMinor).length ?? 0 }}</div>
          <div class="text-gray-600 text-xs mt-1">Minors</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="card space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div><p class="text-gray-600 text-xs">Full Name</p><p class="text-gray-900">{{ profile.user?.fullName }}</p></div>
            <div><p class="text-gray-600 text-xs">Email</p><p class="text-gray-900 truncate">{{ profile.user?.email }}</p></div>
            <div><p class="text-gray-600 text-xs">Phone</p><p class="text-gray-900">{{ profile.user?.phoneNumber }}</p></div>
            <div><p class="text-gray-600 text-xs">Country</p><p class="text-gray-900">{{ profile.country }}</p></div>
            <div><p class="text-gray-600 text-xs">Passport</p><p class="text-gray-900">{{ profile.passportNumber }}</p></div>
            <div><p class="text-gray-600 text-xs">Visa</p><p class="text-gray-900">{{ profile.visaNumber || '—' }}</p></div>
          </div>
        </div>
        <!-- Travel Details Card -->
        <div class="rounded-xl border overflow-hidden"
          ...
          <div class="px-4 py-3 flex items-center justify-between border-b"
            ...
            <div class="flex items-center gap-2">
              ...
              <h2 class="text-sm font-semibold text-gray-900">My Travel Dates</h2>
            </div>
            <button @click="openTravelEdit"
              class="text-xs px-3 py-1.5 rounded-lg border border-emerald-800/50 text-amber-700 hover:bg-emerald-900/30 transition"
              style="touch-action:manipulation; min-height:36px;">
              ✏️ Update
            </button>
          </div>
          <div class="p-4 grid grid-cols-2 gap-3 text-sm">
            <div class="rounded-lg p-2.5" style="background:rgba(212,168,0,0.06);">
              <p class="text-gray-600 text-xs mb-0.5">🛬 Arrival (Iraq)</p>
              <p class="text-gray-900 font-semibold">{{ new Date(profile.arrivalDate).toLocaleDateString('en-GB', {day:'2-digit', month:'short', year:'numeric'}) }}</p>
              <p v-if="profile.arrivalFlight" class="text-amber-700 text-xs mt-0.5">{{ profile.arrivalFlight }}</p>
              <p v-if="itinerary?.arrivalTime" class="text-gray-600 text-xs">{{ itinerary.arrivalTime }}</p>
              <p v-if="itinerary?.arrivalAirport" class="text-gray-600 text-xs truncate">{{ itinerary.arrivalAirport }}</p>
            </div>
            <div class="rounded-lg p-2.5" style="background:rgba(212,168,0,0.06);">
              <p class="text-gray-600 text-xs mb-0.5">🛫 Departure</p>
              <p class="text-gray-900 font-semibold">{{ new Date(profile.departureDate).toLocaleDateString('en-GB', {day:'2-digit', month:'short', year:'numeric'}) }}</p>
              <p v-if="profile.departureFlight" class="text-amber-700 text-xs mt-0.5">{{ profile.departureFlight }}</p>
              <p v-if="itinerary?.departureTime" class="text-gray-600 text-xs">{{ itinerary.departureTime }}</p>
              <p v-if="itinerary?.departureAirport" class="text-gray-600 text-xs truncate">{{ itinerary.departureAirport }}</p>
            </div>
            <div class="col-span-2 rounded-lg p-2.5" style="background:rgba(212,168,0,0.06);">
              <p class="text-gray-600 text-xs mb-0.5">🚌 Bus Assignment</p>
              <p v-if="profile.bus" class="text-amber-700 font-semibold text-sm">Bus {{ profile.bus?.busNumber }} — {{ profile.bus?.driverName || '' }}</p>
              <p v-else class="text-yellow-500 text-xs">Admin will assign your bus — you'll be notified</p>
            </div>
          </div>
          <!-- important note for pilgrim -->
          <div class="px-4 pb-3">
            <p class="text-xs text-gray-700 bg-gray-100 rounded-lg px-3 py-2 border border-gray-200">
      ...
      <div v-if="profile.room" class="rounded-xl border border-primary-800 overflow-hidden"
        ...
        <div class="px-5 py-3 flex items-center gap-2 border-b border-primary-800/50"
             style="background:rgba(212,168,0,0.08);">
          <span class="text-xl">🏨</span>
          <h2 class="text-base font-semibold text-gray-900 flex-1">Your Hotel & Room</h2>
          <span class="text-xs text-amber-700 font-medium">✅ Allocated</span>
        </div>
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="flex-1 min-w-0">
              <p class="text-gray-900 text-xl font-bold leading-tight">{{ profile.room.hotelName || 'Hotel' }}</p>
              <p class="text-amber-700 text-sm mt-0.5 font-medium">{{ profile.room.hotelCity }}</p>
              <p class="text-gray-600 text-xs mt-1">{{ profile.room.hotelAddress }}</p>
            </div>
            <div class="text-right shrink-0 bg-emerald-950/50 border border-emerald-800/50 rounded-xl px-4 py-2">
              ...
              <p class="text-amber-700 font-bold text-2xl">{{ profile.room.roomNumber }}</p>
              <p class="text-gray-600 text-xs">{{ profile.room.floorLabel || `Floor ${profile.room.floorNumber}` }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-2">
            ...
            <a v-if="profile.room.latitude && profile.room.longitude"
               ...
               style="background:linear-gradient(135deg,#b88a00,#D4A800); box-shadow:0 4px 16px rgba(212,168,0,0.25);">
              <span class="text-xl">🧭</span>
            ...
            <div class="grid grid-cols-2 gap-2">
              <a v-if="profile.room.hotelPhone"
                 ...
                 class="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-700 text-gray-600 text-sm transition-colors hover:bg-slate-800">
                📞 Hotel Call
              </a>
      ...
      <div v-if="showCheckoutConfirm" class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
        <div class="bg-white border border-red-300 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
          <div class="text-center mb-5">
            <div class="text-5xl mb-3">🚪</div>
            <h3 class="text-gray-900 text-lg font-bold">Check Out of Room?</h3>
            <p class="text-gray-700 text-sm mt-2">
              You are checking out of <span class="text-gray-900 font-semibold">Room {{ profile.room?.roomNumber }}</span>
              at <span class="text-gray-900 font-semibold">{{ profile.room?.hotelName }}</span>.
            </p>
          ...
          <div class="flex gap-3">
            <button @click="showCheckoutConfirm = false" class="flex-1 py-2.5 rounded-xl border border-gray-300 text-gray-600 hover:text-gray-900 text-sm transition-colors">
              Cancel
            </button>
            <button @click="confirmCheckout" :disabled="checkingOut"
              class="flex-1 py-2.5 rounded-xl bg-red-700 hover:bg-red-600 text-gray-900 font-semibold text-sm transition-colors disabled:opacity-60">
              {{ checkingOut ? 'Checking out...' : 'Yes, Check Out' }}
            </button>
          </div>
        </div>
      </div>
      <div v-else class="rounded-xl border border-gray-200 bg-white px-5 py-4 flex items-center gap-3">
        <span class="text-2xl">🏨</span>
        <div>
          <p class="text-gray-900 font-medium text-sm">Hotel & Room</p>
          <p class="text-gray-600 text-xs mt-0.5">Not allocated yet — admin will assign your room soon.</p>
        </div>
      </div>
      ...
      <div v-if="itinerary" class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold text-gray-900 flex items-center gap-2">✈️ My Itinerary</h2>
          <button @click="openTravelEdit"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <!-- Arrival Card -->
          <div class="rounded-xl border border-green-300 bg-white overflow-hidden">
            <div class="px-4 py-3 bg-green-900/30 border-b border-green-800/50 flex items-center gap-2">
              <span class="text-xl">🛬</span>
              <h3 class="text-gray-900 font-semibold text-sm">Arrival</h3>
            </div>
            <div class="p-4 space-y-3">
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p class="text-gray-600 text-xs">Date</p>
                  <p class="text-gray-900">{{ new Date(itinerary.arrivalDate).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' }) }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Time</p>
                  <p class="text-gray-900">{{ itinerary.arrivalTime || 'Not set' }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Flight</p>
                  <p class="text-gray-900">{{ itinerary.arrivalFlight || '—' }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Airport</p>
                  <p class="text-gray-900">{{ itinerary.arrivalAirport || 'Not set' }}</p>
                </div>
              </div>
              ...
              <div :class="['rounded-lg p-3', (transferStatusConfig[itinerary.arrivalTransfer?.status] || transferStatusConfig.Unassigned).bg]">
                ...
                <template v-if="itinerary.arrivalTransfer?.driverName">
                  <div class="space-y-0.5 text-xs">
                    <p class="text-gray-900">🚗 {{ itinerary.arrivalTransfer.driverName }}
                    ...
                    <p v-if="itinerary.arrivalTransfer.driverPhone" class="text-gray-600">
                      📞 {{ itinerary.arrivalTransfer.driverPhone }}
                      <a :href="'https://wa.me/' + itinerary.arrivalTransfer.driverPhone.replace(/\D/g,'')"
                        target="_blank" class="text-green-600 hover:underline ml-1">WhatsApp</a>
                    </p>
                    <p v-if="itinerary.arrivalTransfer.meetingPoint" class="text-gray-700">
                      📍 {{ itinerary.arrivalTransfer.meetingPoint }}
                    </p>
                    <p v-if="itinerary.arrivalTransfer.scheduledTime" class="text-gray-700">
                ...
                <p v-else class="text-gray-600 text-xs">Admin will assign a driver before your arrival.</p>
              </div>
            </div>
          </div>

          <!-- Departure Card -->
          <div class="rounded-xl border border-blue-300 bg-white overflow-hidden">
            <div class="px-4 py-3 bg-blue-900/30 border-b border-blue-800/50 flex items-center gap-2">
              <span class="text-xl">🛫</span>
              <h3 class="text-gray-900 font-semibold text-sm">Departure</h3>
            </div>
            <div class="p-4 space-y-3">
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <p class="text-gray-600 text-xs">Date</p>
                  <p class="text-gray-900">{{ new Date(itinerary.departureDate).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' }) }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Time</p>
                  <p class="text-gray-900">{{ itinerary.departureTime || 'Not set' }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Flight</p>
                  <p class="text-gray-900">{{ itinerary.departureFlight || '—' }}</p>
                </div>
                <div>
                  <p class="text-gray-600 text-xs">Airport</p>
                  <p class="text-gray-900">{{ itinerary.departureAirport || 'Not set' }}</p>
                </div>
              </div>
              ...
              <div :class="['rounded-lg p-3', (transferStatusConfig[itinerary.departureTransfer?.status] || transferStatusConfig.Unassigned).bg]">
                ...
                <template v-if="itinerary.departureTransfer?.driverName">
                  <div class="space-y-0.5 text-xs">
                    <p class="text-gray-900">🚗 {{ itinerary.departureTransfer.driverName }}
                    ...
                    <p v-if="itinerary.departureTransfer.driverPhone" class="text-gray-600">
                      📞 {{ itinerary.departureTransfer.driverPhone }}
                      <a :href="'https://wa.me/' + itinerary.departureTransfer.driverPhone.replace(/\D/g,'')"
                        target="_blank" class="text-green-600 hover:underline ml-1">WhatsApp</a>
                    </p>
                    <p v-if="itinerary.departureTransfer.meetingPoint" class="text-gray-700">
                      📍 {{ itinerary.departureTransfer.meetingPoint }}
                    </p>
                    <p v-if="itinerary.departureTransfer.scheduledTime" class="text-gray-700">
                ...
                <p v-else class="text-gray-600 text-xs">Admin will assign a driver for your departure.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="showTravelForm" class="card">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-gray-900 font-semibold">Edit Travel Details</h3>
            <button @click="showTravelForm = false" class="text-gray-700 hover:text-gray-900 text-xl">✕</button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
          ...
          <div class="mt-4 flex gap-3">
            ...
            <button @click="showTravelForm = false" class="px-4 py-2 text-sm text-gray-700 hover:text-white">Cancel</button>
          </div>
        </div>
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-gray-600 text-xs mt-0.5">Add all family members traveling with you</p>
          </div>
          <button @click="openAddMember" class="btn-primary text-sm px-3 py-1.5">+ Add Member</button>
        </div>

        <div v-if="showFamilyForm" class="bg-gray-100 rounded-xl p-4 mb-5 border border-gray-200">
          <h3 class="text-gray-900 font-medium mb-3 text-sm">{{ editingMemberId ? 'Edit Family Member' : 'New Family Member' }}</h3>
          ...
          <div class="mt-4 flex gap-2">
            ...
            <button @click="showFamilyForm = false; editingMemberId = null" class="px-4 py-2 text-sm text-gray-700 hover:text-white">Cancel</button>
          </div>
        </div>
        ...
        <div v-if="profile.familyMembers?.length" class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="text-gray-700 border-b border-gray-200 text-xs uppercase">
            ...
            <tbody>
              <tr v-for="m in profile.familyMembers" :key="m.id" class="border-b border-gray-200">
                <td class="py-2.5 text-gray-900 font-medium pr-4">{{ m.fullName }}</td>
                <td class="py-2.5 pr-4">
                  <span :class="m.gender === 1 ? 'text-blue-400' : 'text-pink-400'">
                    {{ m.gender === 1 ? '♂ Male' : '♀ Female' }}
                  </span>
                </td>
                <td class="py-2.5 text-gray-600 pr-4">{{ relationshipOptions.find(r => r.value === m.relationship)?.label }}</td>
                <td class="py-2.5 text-gray-700 pr-4">{{ m.passportNumber || '—' }}</td>
                <td class="py-2.5 pr-4">
                  <span v-if="m.isMinor" class="text-xs bg-gold-900 text-gold-400 rounded px-1.5 py-0.5 mr-1">Minor</span>
                  <span v-if="m.requiresWheelchair" class="text-xs bg-gray-100 text-gray-600 rounded px-1.5 py-0.5">♿</span>
                </td>
                <td class="py-2.5">
        ...
        <div v-else class="text-center py-8 text-gray-700">
